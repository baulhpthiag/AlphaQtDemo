...
class TestParentConfig : public QObject
{
	Q_OBJECT
		...
		Q_PROPERTY(TestChildConfig testChildConfig READ getTestChildConfig WRITE setTestChildConfig)

public:
	TestParentConfig(QObject *parent = nullptr);
	TestParentConfig(const TestParentConfig & other, QObject*parent = nullptr);
	~TestParentConfig();
	...
	//自定义
	TestChildConfig _testChildConfig;
	...
	//自定义
	TestChildConfig& getTestChildConfig();

	friend  QDebug operator << (QDebug debug, const TestParentConfig &config)
	{
	...
	//自定义
	Q_INVOKABLE TestChildConfig* getTestChildConfigP();
	...
	//自定义
	void setTestChildConfig(TestChildConfig& value);

};
Q_DECLARE_METATYPE(TestParentConfig)